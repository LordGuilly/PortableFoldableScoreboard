...
int main(int argc, char ** argv)
{
    ...
    while(1) {
        ...
        if (keyboard_read(NULL, &key_data) == true) {
        	if (key_data.key == LV_KEY_LEFT) {
        		GFX_MGR_ScoreHome();
        	} else if (key_data.key == LV_KEY_RIGHT) {
				GFX_MGR_ScoreAway();
        	} else if (key_data.key == LV_KEY_UP) {
        		GFX_MGR_ChangeColourHome();
        	} else if (key_data.key == LV_KEY_DOWN) {
